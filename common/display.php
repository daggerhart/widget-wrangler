...
class Display {
	...
	/**
	 * Local copy of plugin settings values.
	 *
	 * @var array
	 */
	...
	/**
	 * Copy of the theme compatibility setting.
	 *
	 * @var int
	 */
	public $theme_compat = 0;
	 ...
	 * @var bool
	 */
	public $doing_corral = false;
	public $doing_corral_slug = '';
	public $doing_corral_wp_widget_args = array();
	 ...
	 * @var string
	 */
	public $dynamic_sidebar_index = '';
	public $dynamic_sidebar_args = array();
	public $dynamic_sidebar_has_widgets = FALSE;
	...
	function dynamic_corral($corral_slug = 'default', $wp_widget_args = array('before_widget' => '', 'before_title' => '', 'after_title' => '', 'after_widget' => ''))
	{
		...
		if ( $page_widgets && !is_null($corral_slug) && isset( $page_widgets[$corral_slug] ) ) {
			...
			while($i < $total) {
				$widget = Widgets::get($page_widgets[$corral_slug][$i]['id'], 'publish');
	 ...
	 * @param object - Widget Wrangler widget object
	 *
	 * @return string - advanced parsed widget
	 */
	...
	function single_widget_shortcode( $atts ) {
		...
		// allow slug
		if ( $args['slug'] ) {
			$args['id'] = Utils::getPostIdBySlug( $args['slug'] );
		}

		$widget = Widgets::get( $args['id'], 'publish');
	...
	function corrals_to_wpsidebars_map() {
		...
		$widget_ww_sidebar = get_option( 'widget_widget-wrangler-sidebar', array() );
		...
		foreach ($wp_registered_sidebars as $sidebar_id => $sidebar_details) {
			...
			foreach ($widgets_in_sidebar as $i => $widget_id) {
				$widget_key = str_replace('widget-wrangler-sidebar-', '', $widget_id);

				if (isset($widget_ww_sidebar[$widget_key])){
					$widget_instance = $widget_ww_sidebar[$widget_key];
					$corrals[ $widget_instance['sidebar'] ] = $sidebar_id;
				}
			}
		}
