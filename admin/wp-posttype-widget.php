 ...
 *
 */
class WW_Widget_PostType extends WidgetWranglerAdminPage {
	// public meta fields
	 ...
	 */
	function enqueue() {
		add_filter('manage_edit-widget_columns', array($this, 'tableHeaders'));
		add_action('manage_posts_custom_column', array($this, 'tableData'));
	...
	function add_meta_boxes() {
		...
		if (get_post_type($post) == 'widget')
		{
			...
			add_meta_box('ww-adv-help', __('Help', 'widgetwrangler'), array($this, 'meta_box_help'), 'widget', 'normal', 'high');
	...
	function meta_box_help()
	{
		...
		if (isset($suggestions)) { ?>
			...
			if ($this->widget_type == "clone")
			{
                ...
                <div><pre class="code"><?php print htmlentities(print_r($clone_instance,1)); ?></pre></div>
				<?php
			}
		}
	}    
